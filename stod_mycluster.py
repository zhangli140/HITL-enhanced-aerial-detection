...
cv2.imwrite('predict/my/1_a0.jpg',pimg0)
print(a1,a1.shape)
pimg1 = visualize(a1,3,3)
cv2.imwrite('predict/my/1_a1.jpg',pimg1)
################################################################################
...
def getHeatMap(feature_conv,ac):
    ...
    ac = ac.reshape(nc,3,3)
    for i in range(25):
        h1, w1 = i//5*3, i%5*3
        u = feature_conv[:,h1:h1+3,w1:w1+3]
        for x in range(h1,h1+3):
            for y in range(w1,w1+3):
...
#cv2.imwrite('predict/heatmap/mycluster_1_c0.jpg', result)
heatmap = getHeatMap(features_blobs[0],a1)
img = cv2.imread(testfile)
height, width, _ = img.shape
heatmap = cv2.applyColorMap(cv2.resize(heatmap,(width, height)), cv2.COLORMAP_JET)
result = heatmap * 0.3 + img * 0.5
cv2.imwrite('predict/my/mycluster_1_c1.jpg', result)
#cv2.imwrite('predict/heatmap/mycluster_1_c1.jpg', result)